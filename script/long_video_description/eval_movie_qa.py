...
def eval(model_name, model_data, output_file):
    tot = 0
    acc = 0
    with open('/mnt/bn/lab-agent-hl/heyichen/Audio-Visual/caption_eval/movie_qa_test_en_0703.jsonl') as fin, open(output_file, 'w') as fout:
        for line in fin:
            data = json.loads(line)
            mid = data['id']
            ...
            for qa in data['qa_list']:
                ...
                prompt = load_prompt('/mnt/bn/lab-agent-hl/heyichen/Audio-Visual/caption_eval/answer_en').format(text = movie_description, question=question)
